...
Face = collections.namedtuple("Face", ["bbox", "normed_embedding", "facedata"])
...
class FaceAnalysis:
    ...
    async def get(self, img:np.ndarray, max_size:List[int]=[480, 640], 
        ...
        if not isinstance(boxes, type(None)):
            ...
            #res = []
            #for face in faces:
            #    face_dict = {"identity":"Unknown", "bbox":[]}
            #    face_dict["bbox"]= face.bbox
            #    target_embed = face.normed_embedding
            #    diff = np.subtract(self.saved_embeds, target_embed)
            #    dist = np.sum(np.square(diff), 1)
            #    idx = np.argmin(dist)
            #    if dist[idx] < self.threshold:
            #        face_dict["identity"] = self.names[idx]
            #    res.append(face_dict)
        return faces
